...
// delfirst -> 
// path を p_path に push する前に削除判定を行う
        ...
        */
        if(!p_step.empty()){
            ...
            while(true){
                bool flag = false;
                if(!vec_do[tmp].empty())
                for(auto d = vec_do[tmp].begin(); d != vec_do[tmp].end(); ++d){    
                                ...
                                if(get<1>(**k) == B){
                                    ...
                                    if(R > 0){
                                        if(history_rule2.insert({tmp,B}).second){
                                            ...
                                            if(!vec_do[tmp].empty())
                                            for(auto ind = vec_do[tmp].begin(); ind != vec_do[tmp].end(); ++ind){
                                                if(vec_step[tmp].size() > 0){
                                                    if(al_goal.size() > 0)
                                                    for(auto ini = al_goal.begin(); ini != al_goal.end(); ++ini){
                                                        if(*ini == B){
                                                            al_do.erase(*ind);
                                                            vec_do[tmp].erase(ind);
                                                            flag = true;
                                                            p_inc.push_back({nlist,R,tmp});
                                                            break;
                                                        }
                                                    }
                                                    if(flag)break;
        ...
        if(!p_path.empty()){
            ...
            if(!vec_step[arg3].empty()){
                while(true){
                    bool flag = false;
                if(!vec_do[arg3].empty())
                for(auto d = vec_do[arg3].begin(); d!= vec_do[arg3].end(); ++d){
                            ...
                            if(get<1>(**j) == B){
                                ...
                                if(R > 0){
                                    if(history_rule3.insert({arg3,H,B}).second){
                                        ...
                                        if(!vec_do[arg3].empty())
                                        for(auto ind = vec_do[arg3].begin(); ind != vec_do[arg3].end(); ++ind){
                                            if(vec_step[arg3].size() > 0){
                                                for(auto ini = al_goal.begin(); ini != al_goal.end(); ++ini){
                                                    if(*ini == B){
                                                        al_do.erase(*ind);
                                                        vec_do[arg3].erase(ind);
                                                        flag = true;
                                                        p_inc.push_back({nlist,min,arg3});
                                                        break;
                                                    }
                                                }
                                                if(flag)break;
                            ...
                            if(get<1>(**i) == B){
                                ...
                                if(R>0){
                                    if(history_rule2.insert({ST,B}).second){
                                        ...
                                        if(!vec_do[ST].empty())
                                        for(auto ind = vec_do[ST].begin(); ind != vec_do[ST].end(); ++ind){
                                            if(vec_step[ST].size() > 0){
                                                for(auto ini = al_goal.begin(); ini != al_goal.end(); ++ini){
                                                    if(*ini == B){
                                                        al_do.erase(*ind);
                                                        vec_do[ST].erase(ind);
                                                        flag = true;
                                                        p_inc.push_back({nlist,R,ST});
                                                        break;
                                                    }
                                                }
                                                if(flag)break;
