...
int main(int argc, char *argv[]){
    ...
    */
    ...
    while(true){
        ...
        */
        if(!p_step.empty()){
            auto tmp = p_step.front();
            p_step.pop_front();
            vec_p_step[tmp].pop_front();
            //inc([!S], F, !ST), step(!ST) :- int(F) | step(!New), do(!New).
            bool flag = false;
        ...
        if(!p_path.empty()){
            auto tmp = p_path.front();
            p_path.pop_front();
            auto arg1 = get<0>(tmp);
            auto  H = arg1.front(); 
            auto arg2 = get<1>(tmp);
            auto arg3 = get<2>(tmp);
            vec_p_path[arg3].pop_front();
            // cout << "\tpush path[" ;
            // for(auto l: arg1){
            //     cout << l << "," ;
            // }
