...
int main(int argc, char *argv[]){
    ...
    // 出力用ファイル 
    string filename2 = "result_array_vec_myhash.csv";
    ...
    ihash::hash_table history = ihash::hash_table(INT_MAX/5,5,S);
    s_edge edge_tmp;
    while(true){
        ...
        for (int i = rule2_1[d].size()-1; i >= 0; i--)
        {
            ...
            int nhist[5] = {s,d,target->atom.dst,target->atom.cost,edge_tmp.cost};
        ...
        for (int i = rule2_2[s].size()-1; i >= 0; i--)
        {
            ...
            int nhist[5] = {target->atom.src,s,d,edge_tmp.cost,target->atom.cost};
            if(history.insert(nhist)){
