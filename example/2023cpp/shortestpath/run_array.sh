...
do
    ...
    sed -e 's/VAR/'$count'/g' $DIR/array.cpp > $DIR/array_tmp.cpp

    g++ -o $DIR/array_tmp $DIR/array_tmp.cpp 

    g++ -o $DIR/tmp $DIR/tmp.cpp
    $DIR/tmp > $DIR/input.txt
    
    echo -n $count >> $DIR/result_array.csv
    echo -n , >> $DIR/result_array.csv
