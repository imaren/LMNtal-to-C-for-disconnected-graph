...
// 消すのを優先したうえで、 between は set semantics を持つ前提で、 b_p をリストの配列でなくただの配列に変更
...
int main(int argc, char *argv[]){
    ...
    */
    ...
    list<tuple<int,int,int>>::iterator between_p[n_hl];
    bool between_pf[n_hl];
    ...
    while (!(push_between_ihi.empty())){
        ...
        for(auto i: distance_ihhi1[arg2]){
            ...
            if(history_rule3.insert({arg1,arg2,arg3,get<0>(*i),get<2>(*i),get<3>(*i)}).second){
                ...
                if(between_pf[get<2>(*i)] == true){
                    auto tmp2 = between_p[get<2>(*i)];
                    int max;
                    int arg21 = get<0>(*tmp2);
                    int arg22 = get<1>(*tmp2);
                    int arg23 = get<2>(*tmp2);
                    ...
                    if(arg21 != max || arg23 != min){
                        ...
                        push_between_ihi.erase(tmp2);
                        between_pf[arg22]=false;
                        push_between_ihi.push_back({max,arg22,min});
                        auto titr = prev(push_between_ihi.end());
                        between_p[arg22]=titr;
                        between_pf[arg22]=true;
                    }
                }
                else{
                    push_between_ihi.push_back({AC,get<2>(*i),BD});
                    auto titr = prev(push_between_ihi.end());
                    between_p[get<2>(*i)]=titr;
                    between_pf[get<2>(*i)]=true;
                }
            }  
        }
        for(auto i: distance_ihhi2[arg2]){
            ...
            if(history_rule4.insert({arg1,arg2,arg3,get<0>(*i),get<1>(*i),get<3>(*i)}).second){
                ...
                if(between_pf[get<1>(*i)]== true){
                    auto tmp2 = between_p[get<1>(*i)];
                    int max;
                    int arg21 = get<0>(*tmp2);
                    int arg22 = get<1>(*tmp2);
                    int arg23 = get<2>(*tmp2);
                    ...
                    auto titr = prev(push_between_ihi.end());
                    between_p[get<1>(*i)]=titr;
                    between_pf[get<1>(*i)]=true;
                }
