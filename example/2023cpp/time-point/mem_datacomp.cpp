...
int main(int argc, char *argv[]){
    ...
    */
    ...
    typedef int used; 
    typedef uint32_t hist_id;

    std::map<tuple<int,int,int,int,int,int>, hist_id> idmap;
    hist_id histid = 0;
    ...
    // };
    // const auto comp = [](const hist_type &key)
    // {
    ...
    std::set<hist_id> history_rule3;
    std::set<hist_id> history_rule4; 
    ...
    while(!push_distance_ihhi.empty()){
        ...
        for(auto i: between_ihi[arg2]){
            cout << get<0>(*i) << endl;
            tuple<int,int,int,int,int,int> datatmp = {get<0>(*i),arg2,get<2>(*i),arg1,arg3,arg4};
            auto itrtmp = idmap.find(datatmp);
            if(itrtmp == idmap.end()){
                itrtmp = idmap.insert({datatmp,histid}).first;
                histid++;
            }
            if(history_rule3.insert(itrtmp->second).second){
                int AC = get<0>(*i) + arg1;
                int BD = get<2>(*i) + arg4; 
                push_between_ihi.push_back({AC,arg3,BD});
            }   
            if(history_rule4.insert(itrtmp->second).second){
                int AC = get<0>(*i) - arg4;
                int BD = get<2>(*i) - arg1; 
                push_between_ihi.push_back({AC,arg3,BD});
            }
    ...
    while (!(push_between_ihi.empty())){
        ...
        for(auto i: distance_ihhi1[arg2]){
            tuple<int,int,int,int,int,int> datatmp = {arg1,arg2,arg3,get<0>(*i),get<2>(*i),get<3>(*i)};
            auto itrtmp = idmap.find(datatmp);
            if(itrtmp == idmap.end()){
                itrtmp = idmap.insert({datatmp,histid}).first;
                histid++;
        ...
        for(auto i: distance_ihhi2[arg2]){
            tuple<int,int,int,int,int,int> datatmp = {arg1,arg2,arg3,get<0>(*i),get<1>(*i),get<3>(*i)};
            auto itrtmp = idmap.find(datatmp);
            if(itrtmp == idmap.end()){
                itrtmp = idmap.insert({datatmp,histid}).first;
                histid++;
