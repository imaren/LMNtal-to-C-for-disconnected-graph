...
void print_distance(list<tuple<T,U,U,T,W,W>> in){
    for(auto i: in){
        cout << " distance(" << get<0>(i) << "," << (int)get<1>(i) << "," 
             << (int)get<2>(i) << "," << get<3>(i) << ")."; 
    }
}

template<typename T, typename U, typename W>
void print_between(list<tuple<T,U,T,W>> in){
    for(auto i: in){
        cout << " between(" << get<0>(i) << "," << (int)get<1>(i) << "," 
             << get<2>(i) << ")."; 
    }
}
...
int main(int argc, char *argv[]){
    ...
    */
    ...
    list<list<tuple<int,hl,hl,int,used,used>>::iterator> distance_ihhi1[n_hl];
    ...
    list<list<tuple<int,hl,hl,int,used,used>>::iterator> distance_ihhi2[n_hl];
    list<tuple<int,hl,hl,int>> push_distance_ihhi;
    list<tuple<int,hl,hl,int,used,used>> al_distance_ihhi;
    // vector<list<tuple<int,int,int,used>>::iterator>* between_ihi = new  vector<list<tuple<int,int,int,used>>::iterator>[n_hl];
    list<list<tuple<int,hl,int,used>>::iterator> between_ihi[n_hl];
    list<tuple<int,hl,int>> push_between_ihi;
    list<tuple<int,hl,int,used>> al_between_ihi;
    // list<tuple<int,int,string>> push_min_iil;
    // list<tuple<int,int,string>> push_max_iil;
    list<hl> push_start_h;
    // list<string> other;
    bool flag = true;
    ...
    std::set<hist_type> history_rule3;
    std::set<hist_type> history_rule4; 
    ...
    while(!push_distance_ihhi.empty()){
        tuple<int,hl,hl,int> tmp = push_distance_ihhi.back();
        ...
        for(auto i: between_ihi[arg2]){
            std::stringstream ss;
            ss << get<0>(*i) << ',' << arg2 << ',' << get<2>(*i) << ',' << arg1 << ',' << arg3 << ',' << arg4;
            std::string record = ss.str();
            if(history_rule3.insert(record).second){
                int AC = get<0>(*i) + arg1;
                int BD = get<2>(*i) + arg4; 
                push_between_ihi.push_back({AC,arg3,BD});
            }   
            if(history_rule4.insert(record).second){
                int AC = get<0>(*i) - arg4;
                int BD = get<2>(*i) - arg1; 
                push_between_ihi.push_back({AC,arg3,BD});
            }
    ...
    while (!(push_between_ihi.empty())){
        ...
        tuple<int,hl,int> tmp = push_between_ihi.back();
        int arg1 = get<0>(tmp);
        hl arg2 = get<1>(tmp);
        int arg3 = get<2>(tmp);
        ...
        for(auto i: distance_ihhi1[arg2]){
            std::stringstream ss;
            ss << arg1 << ',' << (int)arg2 << ',' << arg3 << ',' << get<0>(*i)<< ',' << (int)get<2>(*i) << ',' << get<3>(*i);
            std::string record = ss.str();
        ...
        for(auto i: distance_ihhi2[arg2]){
            std::stringstream ss;
            ss << arg1 << ',' << (int)arg2 << ',' << arg3 << ',' << get<0>(*i)<< ',' << (int)get<1>(*i) << ',' << get<3>(*i);
            std::string record = ss.str();
            if(history_rule4.insert(record).second){
                int AC = arg1 - get<3>(*i);
                int BD = arg3 - get<0>(*i); 
