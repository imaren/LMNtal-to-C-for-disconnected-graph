    ...
    */
    ...
    while(!p_m.empty()){
        ...
        if(N > 0){
            ...
            n_hl++;
            p_m.push_back(M);
            p_s.push_back({0,1,0,newlist,n_hl});
            continue;
        }
        al_m.push_back(N);
    }
    ...
    while(!p_job.empty()){
        ...
        al_job.push_back({get<0>(tmpjob),get<1>(tmpjob),get<2>(tmpjob),hlnum});
    ...
    while(true){
        if(!p_s.empty()){
            ...
            //                 int J1 = J+1;
            //                 L.push_front(arg4);
            //                 p_s.push_back({Sum,Time,J1,L,arg4});
            //                 al_job_s.erase(*itr);
            ...
            // //push to atomlist
            // memo ここ？アトムの削除の時の参照関係がバグってないか（他のところも）確かめる
            int hlnum = vec_s[arg4].size()+1;
            al_s.push_back({get<0>(tmps),get<1>(tmps),get<2>(tmps),L,arg4,hlnum});
            ...
            if(!vec_s[arg3].empty()){
                ...
                for(auto itr = vec_s[arg3].begin(); itr != vec_s[arg3].end(); ++itr){
                    ...
                    if(history_rule2.insert({S,J,arg3,arg2}).second){
                        ...
                        int Sum = Weight + S;
                        int J1 = J+1;
                        list<int> L = get<3>(**itr);
                        L.push_front(arg2);
                        al_s.erase(*itr);
                        vec_s[arg3].erase(itr);
                        p_s.push_back({Sum,Time,J1,L,arg3});
                        flag=true;
                        break;
                    }
                }
