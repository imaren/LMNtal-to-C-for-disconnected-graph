...
int main(int argc, char *argv[]){
    ...
    string filename2 = "hashtest2_2.csv";
    ...
    o_file << "SIZE,履歴数,非重複数,重複要素数,重複率" << endl;
    ...
    while (true)
    {
        ...
        for(i_pair i : edge_list[path_tmp.first]){
            ...
            if(history.insert(nhist).second){
                ...
                if(!hash.insert(h3(i.first,i.second,path_tmp.second)).second){
                    dupl++;
                }
    ...
    o_file << S << "," << history.size() << "," << hash.size() << "," << dupl << "," << ((double)dupl/(double)(history.size()))*100 << endl;
