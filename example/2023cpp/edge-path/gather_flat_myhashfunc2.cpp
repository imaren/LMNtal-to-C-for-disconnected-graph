...
int main(int argc, char *argv[]){
    ...
    */
    ...
    // 出力用ファイル 
    string filename2 = "result_hist-flat_hash_myhashfunc2.csv";
    ...
    using hist_type = std::tuple<int,int,int>;
    const auto hash = [](const hist_type &key){
        return (std::get<0>(key)*4027+std::get<1>(key)*4049+std::get<2>(key)*4057);
    };

    ska::flat_hash_set<hist_type, decltype(hash)> history(S,hash); 
    while (true)
    {
